...
- [x] **Frontend Refactoring:** Porting `app.py` away from Streamlit into a Next.js / React application, while exposing the python logic through a `FastAPI` backend.
  - [x] Implement robust REST APIs and WebSocket endpoints in `api.py`.
  - [x] Scaffold Next.js project with TailwindCSS.
  - [x] Migrate Streamlit UI components into reusable React components.
