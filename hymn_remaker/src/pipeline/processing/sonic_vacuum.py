...
class SonicVacuumProcessor:
    ...
    def export_speed_variants(self, audio_array, sample_rate, output_base_path):
        """
        Exports speed variants of the raw staccato hymn audio.
        0.5x (Slower), 1.0x (Base), 2.0x (Faster), 4.0x (Fastest).
        """
        ...
        # 0.5x Speed (Slower) - stretch by factor 2 via linear interpolation
        ...
        # 2.0x Speed (Faster) - take every 2nd sample
        audio_20 = audio_array[::2]
        wavfile.write(f"{output_base_path}_2x.wav", sample_rate, (audio_20 * 32767).astype(np.int16))

        # 4.0x Speed (Fastest) - take every 4th sample
        audio_40 = audio_array[::4]
        wavfile.write(f"{output_base_path}_4x.wav", sample_rate, (audio_40 * 32767).astype(np.int16))

        return [
            f"{output_base_path}_05x.wav",
            f"{output_base_path}_1x.wav",
            f"{output_base_path}_2x.wav",
            f"{output_base_path}_4x.wav"
        ]
