...
class PsyMonoBridge:
    ...
    def separate_stems(self, audio_path, output_dir):
        ...
        cmd = ["python", "-m", "demucs.separate", "--out", output_dir, audio_path]
    ...
    def audio_to_midi(self, audio_path, output_midi_path):
        ...
        if shutil.which("basic-pitch"):
            cmd = ["basic-pitch", out_dir, audio_path]
            try:
                ...
                logger.warning(
                    f"Bridge: Basic-pitch CLI failed, falling back to pYIN: {e}"
                )
    ...
    def harmonize_vocals(
        self, vocal_path, output_dir, target_bpm=145, lyrics_path=None
    ):
        """Step 2.5: Harmonize isolated vocal stem using VocalHarmonizer.

        Generates 2-3 harmonized voice parts (low_harmony, sub_harmony, high_harmony)
        aligned to the target BPM grid, then exports stems + MIDI for Ableton.

        Args:
            vocal_path: Path to isolated vocal WAV stem.
            output_dir: Directory for harmony output.
            target_bpm: BPM for grid alignment (default 145 for psytrance/hip-hop).
            lyrics_path: Optional path to lyrics text for ElevenLabs TTS.

        Returns:
            Dict with "stems" and "midi_path", or empty dict on failure.
        """
        try:
            from hymn_remaker.src.vocal_harmonizer import VocalHarmonizer

            harmonizer = VocalHarmonizer(bpm=target_bpm)
            logger.info(f"Bridge: Harmonizing vocals at {target_bpm} BPM...")

            result = harmonizer.run_pipeline(
            ...
            logger.info(
                f"Bridge: Vocal harmonization complete — "
                f"{len(stems)} stems, MIDI: {midi_path}"
            )
            return {"stems": stems, "midi_path": midi_path}

        except ImportError as e:
            logger.warning(f"Bridge: VocalHarmonizer not available ({e}). Skipping.")
            return {}
        except Exception as e:
            logger.error(f"Bridge: Vocal harmonization failed: {e}")
            return {}
        ...
        try:
            ...
            for track_idx, midi_path in midi_map.items():
                if track_idx < len(live_set.tracks):
                    ...
                    logger.info(
                        f"Bridge: Injecting MIDI into Track {track_idx} ({track.name})"
                    )
                    ...
                    for msg in mid:
                        ...
                        if msg.type == "note_on" and msg.velocity > 0:
                            pending_notes[(msg.note, msg.channel)] = (
                                beat_pos,
                                msg.velocity,
                            )
                            ...
                            if key in pending_notes:
                                ...
                                duration = max(
                                    0.0625, beat_pos - start_beat
                                )  # Min 1/64 note
                                clip.add_note(msg.note, start_beat, duration, velocity)

            # Handle Vocals — run harmonization if available
            harmony_result = {}
            if vocal_path and os.path.exists(vocal_path):
                harmony_dir = os.path.join(
                    os.path.dirname(vocal_path), "..", "harmony_vocals"
                )
                harmony_result = self.harmonize_vocals(
                    vocal_path,
                    harmony_dir,
                    target_bpm=target_bpm,
                )
                if harmony_result.get("stems"):
                    logger.info(
                        f"Bridge: {len(harmony_result['stems'])} harmonized stems loaded "
                        f"for Ableton import"
                    )
                    # Create a track for each harmony stem
                    for voice_name, stem_path in harmony_result["stems"].items():
                        if len(live_set.tracks) > track_idx + 1:
                            track_idx += 1
                            track = live_set.tracks[track_idx]
                            logger.info(f"Bridge: {voice_name} -> Track {track_idx}")
                else:
                    logger.info(f"Bridge: Vocals ready for manual import: {vocal_path}")
    ...
    def run_full_reversal(self, suno_audio_path, output_dir):
        ...
        # 1. Separate
        stem_dir = self.separate_stems(
            suno_audio_path, os.path.join(output_dir, "stems")
        )
        if not stem_dir:
            return False
        ...
        # 2.5 Harmonize vocals
        harmony_dir = os.path.join(output_dir, "harmony_vocals")
        if os.path.exists(vocal_wav):
            harmony_result = self.harmonize_vocals(vocal_wav, harmony_dir)
            vocal_map = harmony_result.get("stems", {})
        else:
            vocal_map = {}

        # 3. Assemble (pass harmony_dir via midi_map for assembly context)
        midi_map_for_assembly = dict(midi_map)
        midi_map_for_assembly["harmony_dir"] = harmony_dir
        return self.assemble_pro_song(midi_map_for_assembly, vocal_wav)
