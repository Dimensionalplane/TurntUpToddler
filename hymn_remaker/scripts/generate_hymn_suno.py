    ...
    for genre_prompt, genre_slug in GENRES:
        for speed in SPEEDS:
            for vocal_mode in VOCAL_MODES:
                ...
                count += 1
                print(
                    f"[{count}/{total}] {genre_slug:15s} @ {speed}x [{vocal_mode:12s}]",
                    flush=True,
                )
                ...
                for attempt in range(1, MAX_RETRIES + 1):
                    ...
                    for i in range(40):
                        ...
                        if "identify" in bt:
                            page.evaluate(
                                """Array.from(document.querySelectorAll('button')).filter(b => b.offsetParent !== null && /full song|instrumental|cover/i.test(b.innerText || '')).forEach(b => b.click())"""
                            )
                            page.evaluate(
                                """Array.from(document.querySelectorAll('button')).find(b => b.offsetParent !== null && (b.innerText || '').trim() === 'Continue')?.click()"""
                            )
                        elif "describe" in bt:
                    ...
                    try:
                        page.evaluate(
                            """Array.from(document.querySelectorAll('[role=\"tab\"]')).find(t => (t.innerText || '').trim() === 'Advanced')?.click()"""
                        )
                    except:
                        pass
                    time.sleep(2)
                    ...
                    for wi in range(20):
                        ...
                        try:
                            created = page.evaluate(
                                """(() => { let btn = Array.from(document.querySelectorAll('button')).find(b => (b.innerText || '').includes('Create') && b.offsetParent !== null && !b.hasAttribute('disabled')); if (btn) { btn.click(); return 'clicked'; } return 'disabled'; })()"""
                            )
                            if created != "disabled":
                                break
                        except:
                        ...
                        if clips and st:
                            ...
                            for di in range(30):
                                try:
                                    ...
                                    if (
                                        r.status_code == 200
                                        and r.json().get("status") == "complete"
                                    ):
                                        ...
                                        if url:
                                            ...
                                            print(
                                                f"  OK {sz}KB -> {os.path.basename(out_mp3)}",
                                                flush=True,
                                            )
                                            break
                                except:
